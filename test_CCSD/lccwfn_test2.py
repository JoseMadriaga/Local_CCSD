...
class lccwfn_test(object):
    ...
    def local_residuals(self, t1_ii, t2_ij):
        ...
        Wmbej_ijim = self.build_lWmbej(Wmbej_ijim, self.ERIoovv_ij, self.ERIovvo_ij, self.ERIovvv_ij, self.ERIoovo_ij, self.Loovv_ij, t1_ii, t2_ij)
        Wmbje_ijim, Wmbie_ijmj = self.build_lWmbje(Wmbje_ijim, Wmbie_ijmj, self.ERIovov_ij, self.ERIovvv_ij, self.ERIoovv_ij, self.ERIooov_ij, t1_ii, t2_ij)        
        r1_ii = self.lr_T1(r1_ii, self.Fov_ij , self.ERIovvv_ij, self.Lovvo_ij, self.Loovo_ij, t1_ii, t2_ij, Fae_ij , Fme_ij, lFmi)
        r2_ij = self.lr_T2(r2_ij,self.ERIoovv_ij, self.ERIvvvv_ij, self.ERIovvo_ij, self.ERIovoo_ij, self.ERIvvvo_ij, self.ERIovov_ij, t1_ii, t2_ij, Fae_ij,lFmi,Fme_ij, lWmnij, Zmbij_ij, Wmbej_ijim, Wmbje_ijim, Wmbie_ijmj)
        ...
        for ij in range(self.no*self.no):
            ...
            for m in range(self.no):
                ...
                for n in range(self.no):
                    ...
                    tmp3 = contract('ab,aA,AZ,bB,BY->ZY', self.H.L[m,n,v,v],Q[ij],L[ij],Q[mn],L[mn])
        ...
        for ij in range(self.no*self.no):
            ...
            for m in range(self.no):
                ...
                for n in range(self.no):
                    ...
                    tmp = contract('ab,aA,AZ,bB,BY->ZY', self.H.L[m,n,v,v],Q[ij],L[ij],Q[nn],L[nn])
    ...
    def build_lWmbej(self, Wmbej_ijim, ERIoovv_ij, ERIovvo_ij, ERIovvv_ij, ERIoovo_ij, Loovv_ij, t1_ii, t2_ij):
        ...
        for ij in range(self.no*self.no):
            ...
            for m in range(self.no):
                ...
                tmp = contract('iabj,aA,AZ->iZbj',self.H.ERI[o,v,v,o], Q[ij],L[ij])
                Wmbej = contract('iZbj,bB,BY->iZYj',tmp,Q[im], L[im])
                ...
                tmp1 = contract('abc,aA,AZ->Zbc',self.H.ERI[m,v,v,v], Q[ij], L[ij])
                tmp2 = contract('Zbc,bB,BY->ZYc',tmp1, Q[im],L[im])
                tmp3 = contract('ZYc,cC,CX->ZYX',tmp2, Q[jj], L[jj])
                Wmbej_1[m,:,:,j] += contract('F,beF->be', t1_ii[j], tmp3)
                ...
                for n in range(self.no):
                    ...
                    tmp4 = Sijnn @ t1_ii[n] 
                    Wmbej_2[m,:,:,j] -= contract('b,e->be',tmp4,ERIoovo_ij[im][m,n,:,j]) 
 
                    Sijjn = L[ij].T @ Q[ij].T @ Q[jn] @ L[jn]
            
                    tmp5 = self.build_ltau(jn,t1_ii,t2_ij, 0.5, 1.0) @ Sijjn.T 
                    tmp6 =contract('ab,aA,AZ,bB,BY->ZY', self.H.ERI[m,n,v,v],Q[im],L[im],Q[jn],L[jn])
                    Wmbej_3[m,:,:,j] -= contract('Fb,eF->be', tmp5, tmp6)

                    Sijnj = L[ij].T @ Q[ij].T @ Q[nj] @ L[nj]
                    
                    tmp7 = t2_ij[nj] @ Sijnj.T 
                    tmp8 = contract('ab,aA,AZ,bB,BY->ZY', self.H.L[m,n,v,v],Q[im],L[im],Q[nj],L[nj])
                    Wmbej_4[m,:,:,j] += 0.5 * contract('Fb,eF->be', tmp7, tmp8)
                    
                #print("Wmbje_ijim",ij,m, ij*self.no + m)
                
                Wmbej_ijim.append(Wmbej + Wmbej_1 + Wmbej_2 + Wmbej_3 + Wmbej_4)
        return Wmbej_ijim
    ...
    def build_lWmbje(self, Wmbje_ijim,Wmbie_ijmj,ERIovov_ij, ERIovvv_ij, ERIoovv_ij, ERIooov_ij, t1_ii, t2_ij):
        ...
        for ij in range(self.no*self.no):
            ...
            for m in range(self.no):
                ...
                tmp = contract('iajb,aA,AZ->iZjb',self.H.ERI[o,v,o,v], Q[ij], L[ij])
                Wmbje = -1.0 * contract('iZjb,bB,BY->iZjY', tmp, Q[im], L[im])
                ...
                tmp1 = contract('abc,aA,AZ->Zbc',self.H.ERI[m,v,v,v], Q[ij], L[ij])
                tmp2 = contract('Zbc,bB,BY->ZYc',tmp1, Q[jj],L[jj])
                tmp3 = contract('ZYc,cC,CX->ZYX',tmp2, Q[im], L[im])
                Wmbje_1[m,:,j,:] -= contract('f,bFe->be', t1_ii[j], tmp3)
                ...
                for n in range(self.no):
                    ...
                    tmp4 = Sijnn @ t1_ii[n]
                    Wmbje_2[m,:,j,:] += contract('b,e->be',tmp4,ERIooov_ij[im][m,n,j])

                    Sijjn = L[ij].T @ Q[ij].T @ Q[jn] @ L[jn]
                    
                    tmp5 = self.build_ltau(jn,t1_ii,t2_ij, 0.5, 1.0) @ Sijjn.T
                    tmp6 = contract('ab,aA,AZ,bB,BY->ZY', self.H.ERI[m,n,v,v],Q[jn],L[jn],Q[im],L[im])
                    Wmbje_3[m,:,j,:] += contract('Fb,Fe->be', tmp5, tmp6)
        ...
        for ij in range(self.no*self.no):
            ...
            for m in range(self.no):
                ...
                tmp = contract('iajb,aA,AZ->iZjb',self.H.ERI[o,v,o,v], Q[ij], L[ij])
                Wmbie = -1.0 * contract('iZjb,bB,BY->iZjY', tmp, Q[mj], L[mj])
                ...
                tmp1 = contract('abc,aA,AZ->Zbc',self.H.ERI[m,v,v,v], Q[ij], L[ij])
                tmp2 = contract('Zbc,bB,BY->ZYc',tmp1, Q[ii],L[ii])
                tmp3 = contract('ZYc,cC,CX->ZYX',tmp2, Q[mj], L[mj])
                Wmbie_1[m,:,i,:] -= contract('f,bFe->be', t1_ii[i], tmp3)
                ...
                for n in range(self.no):
                    ...
                    tmp4 = Sijnn @ t1_ii[n]
                    Wmbie_2[m,:,i,:] += contract('b,e->be',tmp4,ERIooov_ij[mj][m,n,i])

                    Sijin = L[ij].T @ Q[ij].T @ Q[_in] @ L[_in]

                    tmp5 = self.build_ltau(_in,t1_ii,t2_ij, 0.5, 1.0) @ Sijin.T
                    tmp6 = contract('ab,aA,AZ,bB,BY->ZY', self.H.ERI[m,n,v,v],Q[_in],L[_in],Q[mj],L[mj])
                    Wmbie_3[m,:,i,:] += contract('Fb,Fe->be', tmp5, tmp6)
    ...
    def lr_T1(self, r1_ii, Fov_ij , ERIovvv_ij, Lovvo_ij, Loovo_ij, t1_ii, t2_ij, Fae_ij , Fme_ij, lFmi):
        ...
        for i in range(self.no):
            ...
            for m in range(self.no):
                ...
                tmp2 = contract('abc,aA,AZ->Zbc',self.H.ERI[m,v,v,v], Q[ii], L[ii])
                tmp3 = contract('Zbc,bB,BY->ZYc',tmp2, Q[mi],L[mi])
                tmp4 = contract('ZYc,cC,CX->ZYX',tmp3, Q[mi], L[mi]) 
            ...
            for n in range(self.no):
                ...
                tmp5 = contract('ab,aA,AZ,bB,BY->ZY', self.H.L[n,v,v,i],Q[ii],L[ii],Q[nn],L[nn])
                r1_4 += contract('F,aF->a', t1_ii[n], tmp5)
    ...
    def lr_T2(self,r2_ij,ERIoovv_ij, ERIvvvv_ij, ERIovvo_ij, ERIovoo_ij, ERIvvvo_ij, ERIovov_ij, t1_ii, t2_ij, Fae_ij,lFmi,Fme_ij, lWmnij, Zmbij_ij, Wmbej_ijim, Wmbje_ijim, Wmbie_ijmj):
        v = self.v
        Q = self.Local.Q
        L = self.Local.L
        ...
        for ij in range(self.no*self.no):
            ...
            for m in range(self.no):
                ...
                r2_6 += contract('aE,bE->ab',tmp5, Wmbej_ijim[ijm][m,:,:,j]) 
               
                tmp6 = Sijim @ t2_ij[im] 
                tst  = Wmbje_ijim[ijm].swapaxes(2,3) 
                tmp7 = Wmbej_ijim[ijm][m,:,:,j] + tst[m,:,:,j]
                ...
                tmp8 = Sijmj @ t2_ij[mj] 
                tmp9 = Wmbie_ijmj[ijm][m,:,i,:] 
                r2_8 += contract('aE,bE->ab',tmp8,tmp9)
  
                tmp10 = contract('ab,aA,AZ->Zb',self.H.ERI[m,v,v,j], Q[ij],L[ij])
                tmp11 = contract('Zb,bB,BY->ZY',tmp10,Q[ii], L[ii])
                tmp12 = contract ('E,a->Ea', t1_ii[i],tmp)
                r2_9 -= contract('Ea,bE->ab',tmp12, tmp11)
                
                tmp13 = contract('ab,aA,AZ->Zb',self.H.ERI[m,v,j,v], Q[ij], L[ij])
                tmp14 = contract('Zb,bB,BY->ZY', tmp13, Q[ii], L[ii])
                r2_10 -= contract('Eb,aE-> ab', tmp12,tmp14)

                tmp15 = contract('abc,aA,AZ->Zbc',self.H.ERI[v,v,v,j], Q[ij], L[ij])
                tmp16 = contract('Zbc,bB,BY->ZYc',tmp15, Q[ij], L[ij])
                tmp17 = contract('ZYc,cC,CX->ZYX',tmp16, Q[ii], L[ii])
        ...
        for ij in range(self.no*self.no):
            ...
            r2_ij[ij] += r2_ij[ji].swapaxes(0,1)    
