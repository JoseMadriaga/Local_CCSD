    ...
    def _local_MP2_loop(self,local,S):
        ...
        # t2_ij = pno_o by pno_o 
            ...
            # Construct the modified MP basis of the ERI and transform it into the PAO basis
                ...
                # below gives the same results as the above code for ERI transformation with either t2 being built independently or built from this transformed ERI
                #S_ij = L[ij] @ Q[ij] @ Q[ij].T @ L[ij].T
                #ERI_ij.append( contract('rs,ar,bs->ab', self.H.ERI[i,j,v,v], S_ij, S_ij))
                #ERI_ij[ij] = L[ij].T @ Q[ij].T @ ERI_ij[ij] @ Q[ij] @ L[ij]                

                # another attempt
                ERI_ij.append(L[ij].T @ Q[ij].T @ self.H.ERI[i,j,v,v] @ Q[ij] @ L[ij])
                S_ij = L[ij].T @ Q[ij].T @ Q[ij] @ L[ij]
                ERI_ij[ij] = contract('rs,ar,bs->ab', ERI_ij[ij], S_ij, S_ij)

                # t2_ij = pno_o by pno_o  
                t2_ij.append( -1.0 * ERI_ij[ij] / (self.eps[ij].reshape(1,-1) + self.eps[ij].reshape(-1,1) - self.H.F[i,i] - self.H.F[j,j]))
                #t2_ij.append(L[ij].T @ Q[ij].T @ t2[i,j] @ Q[ij] @ L[ij])    
        ...
        if local.upper() in ["LPNO"]:
            while ((abs(ediff) > e_conv) or (abs(rmsd) > r_conv)) and (niter <= maxiter):
                ...
                for ij in range(self.no*self.no):  
                    ...
                    #for a in range(self.dim[ij]):
                        #for b in range(self.dim[ij]):
                            #r2_ij[a,b] += (self.eps[ij][a] - self.eps[ij][b])*t2_ij[ij]

                    for k in range(self.no):
                        if i != k: 
                            kj = self.no * k + j 
                            # without the L, it's over by 0.001. with L its under by 0.003
                ...
                r2_sa = 0.5 * self.H.ERI[o,o,v,v]
           
                r2_sa += contract('ijac,cb->ijab', t2_sa, self.H.F[v,v])

                r2_sa -= contract('imab,mj->ijab', t2_sa, F_occ) 
                
                r2_sa = r2_sa + r2_sa.swapaxes(0,1).swapaxes(2,3)
 
                t2_sa += r2_sa/Dijab                

                emp2_sa = contract('ijab,ijab->', t2_sa, self.H.L[o,o,v,v])
                
                print("MP2 Iter %3d: MP2 Ecorr = %.15f dE = % .5E" % (niter, emp2_sa, ediff_sa))
                print(emp2 - emp2_sa) 
