...
#                    break
    ...
    def _sim_MP2_loop(self,local):
        ...
        while ((abs(ediff) > e_conv) or (abs(rmsd) > r_conv)) and (niter <= maxiter):
            ...
            #This should be right since the untruncated mp2 convergences to the 1e-15 
            ...
            #new_tijab = self.filter_ruhsim(r2,F_occ)           

            new_tijab = self.filter_mine(r2)            
    ...
    def filter_mine(self,r2):
        ...
        for ij in range(self.no * self.no):
            ...
            r2_trans = self.L[ij].T @ self.Q[ij].T @ r2[i,j] @ self.Q[ij] @ self.L[ij]

            inc = r2_trans / (self.eps[ij].reshape(-1,1) + self.eps[ij].reshape(1,-1) - self.H.F[i,i] - self.H.F[j,j])
            #if ij == 0:
                #print('inc from filter')
                #print(inc)
            
            t2[i,j] -= self.Q[ij] @ self.L[ij] @ inc  @ self.L[ij].T @ self.Q[ij].T

        return t2

    def filter_ruhsim(self,r2,F_occ):
        no = self.no
        nv = self.nv 
        ...
        for ij in range(self.no * self.no):
            ...
            R2Q = contract('ca,ab,bd->cd', tmp1.T, r2[ij // self.no, ij % self.no], tmp1)
            print("R2Q")
            print(R2Q)
            ...
            for a in range(tmp3.shape[0]):
                for b in range(tmp3.shape[0]):
                    d2_QL[a, b] = F_occ[ij // self.no, ij // self.no ] + F_occ[ij % self.no, ij % self.no] - tmp3[a] - tmp3[b]
            ...
            t2[ij // self.no, ij % self.no] += contract('ca,ab,bd->cd', tmp1, T2Q, tmp1.T)
