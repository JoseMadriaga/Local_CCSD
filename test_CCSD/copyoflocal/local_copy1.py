...
#                    break
    ...
    def _local_MP2_loop(self,local,S):
        ...
        # Transforming the ao overlap matrix to localized MO overlap matrix
        #S = self.C.T @ S @ self.C
        #print('localized MO overlap')
        #print(S)
        ...
        if local.upper() in ["PAO"]:
         
            # Initializing list to store each pair ij transformations
            # C_ij is ao by pno
            C_ij = []
            S_ij = []
            F_ij = []
            ...
            # Running through pair ij 
            for ij in range(self.no*self.no):
                i = ij // self.no
                j = ij % self.no
  
            # Transformed virtual overlap matrix
                C_ij.append(self.C @ Q[ij])
                S_ij.append(C_ij[ij].T @ S @ C_ij[ij])
                #S_ij[ij] = np.linalg.norm(S_ij[ij])*S_ij[ij]
                #print('transformed virtual overlap')
                #print(S_ij[ij])

            # Transform the virtual block of Fock matrix then folding in the 
            # overlap terms ( F = F[ij]_ef where all virtual indices are nonorthogonal such that 
            # F_ij = F[ij]_ab = F[ij]_ef S[ij]_ea S[Iij]_fb)   
                F = Q[ij].T @ self.H.F[v,v] @ Q[ij]
                F_ij.append(contract('ef,ea,fb->ab',F,S_ij[ij],S_ij[ij]))

            # Transform the virtual block of ERI into the PAO basis then folding in the overlap
            # terms associated with ERI ( ERI = ERI[ij]_ef S[ij]_ea S[Iij]_fb)
                ERI = Q[ij].T @ self.H.ERI[i,j,v,v] @ Q[ij]
                ERI_ij.append(contract('ef,ea,fb->ab',ERI,S_ij[ij],S_ij[ij]))
  
            # Transform the L matrix ( 2g - 2g.axes(2,3)) into the PAO basis 
                L_all = 2*self.H.ERI - self.H.ERI.swapaxes(2,3)
                L_ij.append(Q[ij].T @ L_all[i,j,v,v] @ Q[ij]) 
                        
                # Transform the t2 into the PAO basis
                #t2_pao[i,j] = Q[ij].T @ t2[i,j] @ Q[ij]
                t2_ij.append(Q[ij].T @ t2[i,j] @ Q[ij])
        ...
        # C_ij = ao by pno_nr       
            ...
            # Transformed virtual-virtual block of the overlap and Fock matrix to the PAO/PNO
            for ij in range(self.no*self.no):
                i = ij // self.no
                j = ij % self.no
            
            # need to transform the AO overlap to MO overlap first
            # Transformed virtual-virtual overlap matrix for this specific pair domain
                ...
                # S_ij = pao_nr by pao_nr
                S_ij.append(C_ij[ij].T @ S @ C_ij[ij])
                #print('transformed virtual overlap')
                print(S_ij[ij])
                #S_ij[ij] = S_ij[ij]/np.linalg.norm(S_ij[ij])           
 
                # S_v_pno = vmo by pno_red
                Sij_v_pnored = self.C[:,v].T @ S @ self.C[:,v]
                Sij_v_pnored = Sij_v_pnored @ Q[ij]
                #Sij_v_pnored = Sij_v_pnored/np.linalg.norm(Sij_v_pnored)               

            # Construct the modifed MO basis of the Fock matrix and transform it into the PAO basis
                
                #F_ij.append(Sij_v_pnored.T @ self.H.F[v,v] @ Sij_v_pnored)
                #print('F_ij redundant')
                #print(F_ij[ij])
                F_ij.append(Q[ij].T @ self.H.F[v,v] @ Q[ij])               
                print('F_ij redundant')
                print(F_ij[ij])
                F_ij[ij] = L[ij].T @ F_ij[ij] @ L[ij]
                print('diagonal external Fock matrix')
                print(F_ij[ij]) 
                print(self.eps[ij])
            
            # Construct the modified MP basis of the ERI and transform it into the PAO basis
                ERI_ij.append(Sij_v_pnored.T @ self.H.ERI[i,j] @ Sij_v_pnored)
                #ERI_ij.append(Q[ij].T @ self.H.ERI[i,j,v,v] @ Q[ij])
                ERI_ij[ij] = L[ij].T @ ERI_ij[ij] @ L[ij]
     
            # Transform the L matrix ( 2g - 2g.axes(2,3)) into the PAO basis
                L_all = 2*self.H.ERI - self.H.ERI.swapaxes(2,3)
                L_ij.append(Q[ij].T @ L_all[i,j,v,v] @ Q[ij])
                L_ij[ij] = L[ij].T @ L_ij[ij] @ L[ij]

                # Transform the t2 into the PAO basis
                #t2_pao[i,j] = Q[ij].T @ t2[i,j] @ Q[ij]
                t2_ij.append(Q[ij].T @ t2[i,j] @ Q[ij])    
                t2_ij[ij] = L[ij].T @ t2_ij[ij] @ L[ij] 
        ...
        if local.upper() in ["PAO"]:
            ...
            # MP2 pair loop in the local basis
                for ij in range(self.no*self.no):
                    ...
                    r2_ij = ERI_ij[ij]
                    r2_ij += contract('cd,ad,cb->ab', t2_ij[ij],F_ij[ij], S_ij[ij])
                    r2_ij += contract('cd,ac,db->ab', t2_ij[ij],F_ij[ij], S_ij[ij])
                    # I'm having trouble doing the next two lines
                    ...
                    for k in range(self.no):
                        ...
                        t2_ik = S_ijik @ t2_ij[ik] @ S_ijik.T
                        t2_kj = S_ijkj @ t2_ij[kj] @ S_ijkj.T 
  
                        r2_ij -= contract('cd,li,ca,db->ab', t2_kj, self.H.F[o,o], S_ij[ij], S_ij[ij])
                        r2_ij -= contract('cd,ki,ca,db->ab', t2_ik, self.H.F[o,o], S_ij[ij], S_ij[ij])
		
                        if k == 4:
                            print('next ij')

                    r2_ij = L[ij].T @ r2_ij @ L[ij]
		    
                    for a in range(self.dim[ij]):
                        for b in range(self.dim[ij]):
                            r2_ij[a,b] = r2_ij[a,b]/(self.H.F[i,i] + self.H.F[j,j] - self.eps[ij][a] - self.eps[ij][b])

                    r2_ij = L[ij] @ r2_ij @ L[ij].T
        ...
        if local.upper() in ["LPNO"]:
            while ((abs(ediff) > e_conv) or (abs(rmsd) > r_conv)) and (niter <= maxiter):
                ...
                #zach's version with some some modification
                for ij in range(self.no*self.no):  
                    i = ij // self.no 
                    j = ij % self.no
                    ...
                    # transforming back to MO basis for now to compute emp2 energy
