        ...
        if local.upper() in ["LPNO"]:
            ...
            # MP2 pair loop in the local basis
                for ij in range(self.no*self.no):
                    ...
                    r2_ij = ERI_ij[ij]
                    r2_ij += contract('bd,ad->ab', t2_ij[ij],F_ij[ij])
                    r2_ij += contract('cb,ac->ab', t2_ij[ij],F_ij[ij])


                    # Expression to code are t[kl]_ab F[ij]_li ( this is the first contraction below
                    # in the k loop) and t[ik]_ab F[ij]_ki ( this is the second contraction)  
			...
			# generate a projection matrix to project from kj or ik pair domain to ij 
                        #S_ijik = C_ij[ij].T @ S @ C_ij[ik]
                        #S_ijkj = C_ij[ij].T @ S @ C_ij[kj]
                        ...
                        # Contracting t[kl]_ab F[ij]_li to get r2[ij]_ab
